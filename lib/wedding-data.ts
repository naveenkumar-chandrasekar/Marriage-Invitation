export const couple = {
  person1: { name: "Naveenkumar C" },
  person2: { name: "Keerthiga T" },
};

// ISO string, read by the countdown — the wedding ceremony date.
export const weddingDateISO = "2027-03-12T00:00:00";

export const weddingDateLabel = "Friday, 12 March 2027";

export const events = [
  {
    title: "Engagement",
    icon: "💍",
    description:
      "The formal start of our forever — join us as our families come together to bless this union with the exchange of rings.",
    date: "Sunday, 13 December 2026",
    locations: [
      {
        label: "Venue",
        name: "DSP Thirumana Mahal",
        address: "Deviyakurichi, Attur, Tamil Nadu 636112",
        time: "10:00 AM – 1:00 PM",
        mapUrl: "https://maps.app.goo.gl/gYSocyAjzzFe6EwF6",
      },
    ],
  },
  {
    title: "Marriage",
    icon: "🛕",
    description:
      "Our sacred wedding ceremony, with rituals at the temple followed by celebrations at the venue. Your presence would mean the world to us.",
    date: "Friday, 12 March 2027",
    locations: [
      {
        label: "Temple",
        name: "Arulmigu Thiru Ardhanareeswarar Temple",
        address: "Arthanareeswarar Hill Temple Road, Tiruchengode, Tamil Nadu 637205",
        time: "5:00 AM – 7:00 AM",
        mapUrl: "https://maps.app.goo.gl/WnJQ8dQ53ReLbtot6",
      },
      {
        label: "Venue",
        name: "SKV Mahal",
        address: "Tiruchengode, Tamil Nadu 637205",
        time: "7:00 AM – 9:00 AM",
        mapUrl: "https://maps.app.goo.gl/JwXqNrBmF38A1mnMA",
      },
    ],
  },
  {
    title: "Reception",
    icon: "🎉",
    description:
      "An evening of food, music, and celebration as we begin our new chapter together — come raise a toast with us.",
    date: "Sunday, 14 March 2027",
    locations: [
      {
        label: "Venue",
        name: "Komarasamy Gounder Nallammal Marriage A/C Hall",
        address: "Salem - Chennai Highway, Mammudi, Tamil Nadu 636112",
        time: "11:00 AM – 3:00 PM",
        mapUrl: "https://maps.app.goo.gl/SsdK92nG7mRcgKaWA",
      },
    ],
  },
];
